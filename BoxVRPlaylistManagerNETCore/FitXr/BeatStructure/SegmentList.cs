...
namespace BoxVRPlaylistManagerNETCore.FitXr.BeatStructure
{
    public class SegmentList
    {
        ...
        private void CalculateSegmentEnergies(ref List<Segment> segments, List<BeatInfo> beats)
        {
            ...
            segmentList.Sort((Comparison<Segment>)((x, y) => (int)Math.Sign(x._averageEnergy - y._averageEnergy)));
        ...
        private int FindLargestNeighbourEnergyDelta(List<Bar> bars, int centerBarIndex, int range)
        {
            ...
            for(int index = 0; index < length; ++index)
                array[index] = Math.Abs(numArray[index + 1] - numArray[index]);
          ...
          BarList bars)
        {
            for(int index1 = 0; index1 < segments.Count; ++index1)
            {
                ...
                int index2 = Math.Clamp(closestBar + neighbourEnergyDelta, 0, bars._bars.Count - 1);
