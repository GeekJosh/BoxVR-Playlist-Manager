...
using BoxVRPlaylistManagerNETCore.FitXr.MusicActions;
using log4net;
...
namespace BoxVRPlaylistManagerNETCore.FitXr.Models
{
    ...
    {
        private ILog _log = LogManager.GetLogger(typeof(SongDefinition));
        ...
        {
            _log.Debug("Editing " + (object)channel + "/" + (object)beatNumber + " = " + (object)moveType);
            ...
            if(moveType == MoveType.None)
            {
                if(index1 != -1)
                {
                    _log.Debug("Deleting entry at " + (object)index1);
